...
当服务器要求额外的键盘交互输入、OTP 或重新认证时，NyaTerm 用专用的 SSH 认证请求窗口收集信息，而不把提示混在终端输出里。窗口会区分这几种情况：
...
**网络设备** 用于交换机、路由器等非 Linux shell 的设备 CLI。选择后，NyaTerm 会在运行时关闭 SFTP 浏览、目录跟随、Shell 检测、Shell Integration、远程资源统计和自动图标识别，避免把设备 CLI 当作完整 shell 探测；这个选择不会改写已保存的 SFTP 选项。
...
选择 **NyaTerm JSON** 可以导入一个 `.json` 文件，用于从脚本、资产清单或其他系统批量生成会话数据。
