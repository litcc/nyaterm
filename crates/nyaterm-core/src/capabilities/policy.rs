...
pub fn decide_policy(
    ...
    match mode {
        AiPermissionMode::Observer => match access {
            CapabilityAccess::Read => PolicyDecision::Allow,
            CapabilityAccess::SensitiveRead => PolicyDecision::RequireApproval,
            CapabilityAccess::Write | CapabilityAccess::DestructiveWrite => PolicyDecision::Deny,
        },
        AiPermissionMode::Confirm => match access {
            CapabilityAccess::Read => PolicyDecision::Allow,
            CapabilityAccess::SensitiveRead
            | CapabilityAccess::Write
            | CapabilityAccess::DestructiveWrite => PolicyDecision::RequireApproval,
        },
        AiPermissionMode::Auto => match access {
            CapabilityAccess::Read | CapabilityAccess::SensitiveRead => PolicyDecision::Allow,
            CapabilityAccess::Write => {
                if assessment
                    .is_some_and(|risk| risk.auto_executable && risk.level < RiskLevel::High)
                {
        ...
        AiPermissionMode::FullAccess => PolicyDecision::Allow,
    }
}
...
mod tests {
    ...
    fn permission_matrix_is_conservative() {
        ...
        assert_eq!(
            decide_policy(&AiPermissionMode::Auto, CapabilityAccess::Write, None),
            PolicyDecision::RequireApproval
        );
        assert_eq!(
            decide_policy(
